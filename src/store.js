...
export class MemoryStore {
  ...
  async hydrate(types=['agents','projects','tasks','approvals','memory','runs','executions','verifications','tools','artifacts']) { if(!hasD1(this.env))return false; for(const type of types){const rows=await d1List(this.env,type);const existing=this.data.get(type)??new Map();for(const item of rows)if(item?.id)existing.set(item.id,item);if(existing.size)this.data.set(type,existing);}this.events=await d1Events(this.env);this.hydrated=true;return true; }
