...
async function generateFunctionalArtifact({ task, env, agentId }) {
  ...
  if (!runtimeEnv?.AI?.run) {
    ...
    // Stub if no template matches
    const stubFiles = [{ path: 'README.md', content: '# ' + objective + '\n\nGenerated by MAULI 2.0. AI binding unavailable.' }];
    const artifact = registerArtifact({
      projectId: task.projectId, taskId: task.id, agentId, type: 'code-workspace',
      content: { summary: 'Stub: AI unavailable for ' + objective, files: stubFiles, tests: [], notes: ['AI binding unavailable'] },
      metadata: { generatedBy: 'functional-code-executor', stub: true, fileCount: 1 }
    });
