...
export async function createCommandRecord(env,text){return persist(env,{id:`cmd_${crypto.randomUUID()}`,command:text,state:'accepted',phase:'accepted',progress:{percent:0,completed:0,total:0,currentTask:null},lifecycle:['accepted'],createdAt:now(),updatedAt:now(),result:null,resultFile:null,error:null})}
export async function runCommandStep(env,commandId){
 let command=await read(env,commandId);if(!command||['completed','failed'].includes(command.state))return command;
 ...
 try{
  if(!project){const prepared=await withTimeout(prepareCommand(command.command,env),MAX_STEP_MS);if(prepared?.status==='awaiting_approval')return persist(env,{...command,state:'awaiting_approval',phase:'awaiting_approval',projectId:prepared.project?.id??null,lifecycle:[...new Set([...command.lifecycle,'awaiting_approval'])],updatedAt:now(),result:prepared,resultFile:null});project=prepared.project;command={...command,projectId:project?.id??null}}
  ...
  if(!current){const terminal=project.state==='completed'||snap.progress.percent===100;const out={project,tasks:snap.tasks,progress:snap.progress,status:terminal?'completed':'failed'};const rf=await saveCommandResult({command:command.command,commandId:command.id,generatedAt:now(),result:out},env);return persist(env,{...command,state:terminal&&rf.saved?'completed':'failed',phase:terminal&&rf.saved?'delivery':'failed',progress:snap.progress,updatedAt:now(),result:out,resultFile:rf,error:terminal?(rf.saved?null:rf.reason):'No executable task remains',lifecycle:[...new Set([...command.lifecycle,terminal?'delivery':'failed'])]})}
  ...
  project=result?.project??store.get('projects',project.id);const after=projectSnapshot(project),p=phase(project,after);const done=project.state==='completed'||(after.progress.percent===100&&after.tasks.every(t=>t.state==='completed'));const out={project,tasks:after.tasks,progress:after.progress,status:result?.status??(done?'completed':'active'),verification:result?.verification??null,execution:result?.execution??null,finalDelivery:result?.finalDelivery??null};const rf=await saveCommandResult({command:command.command,commandId:command.id,generatedAt:now(),result:out},env);const finished=done&&rf.saved;
  return persist(env,{...command,state:finished?'completed':(result?.status==='blocked'||result?.status==='escalated'?'failed':'running'),phase:finished?'delivery':p,projectId:project.id,progress:after.progress,lifecycle:[...new Set([...command.lifecycle,p,finished?'completed':'checkpoint'])],updatedAt:now(),result:out,resultFile:rf,error:finished?null:(rf.saved?null:rf.reason)});
 }catch(error){const snap=project?projectSnapshot(project):{progress:command.progress,tasks:[]};const recoverable=/checkpoint saved|timeout/i.test(error?.message||'');const out={project,tasks:snap.tasks,progress:snap.progress,status:'checkpoint',error:error?.message||String(error)};let rf=null;try{rf=await saveCommandResult({command:command.command,commandId:command.id,generatedAt:now(),result:out,error:out.error},env)}catch{}return persist(env,{...command,state:recoverable?'running':'failed',phase:recoverable?'recovering':'failed',projectId:project?.id??command.projectId??null,progress:snap.progress,lifecycle:[...new Set([...command.lifecycle,recoverable?'recovering':'failed'])],updatedAt:now(),result:out,resultFile:rf,error:recoverable?null:out.error})}
}
