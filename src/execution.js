...
export async function executeTask(task,context={}) {
  ...
  // A live task may have only one execution lease. Even forced retries must not create a second live run.
  if(existing){
...
export async function executeTaskLifecycle(task,context={}) {
  if(!task?.id)return{status:'failed',error:'Task is required'};let currentTask=store.get('tasks',task.id)??task;const approval=grantedApprovalForTask(currentTask);const lifecycleContext={...context,approved:context.approved||Boolean(approval),approvalId:context.approvalId??approval?.id};if(currentTask.agentId==null&&currentTask.assignedAgentId!=null)currentTask=store.put('tasks',{...currentTask,agentId:currentTask.assignedAgentId,id:currentTask.id});
  if(currentTask.state==='blocked'){const ready=(currentTask.dependsOn??[]).every(depId=>store.get('tasks',depId)?.state==='completed');if(!ready&&!lifecycleContext.dependenciesComplete)return{status:'blocked',task:currentTask,reason:currentTask.blockedReason??'Dependencies incomplete'};currentTask=store.put('tasks',{...currentTask,state:'queued',blockedReason:null,id:currentTask.id});}
  if(currentTask.state==='completed')return completedLifecycleResponse(currentTask);if(currentTask.state==='verifying')return{status:'verifying',task:currentTask};if(currentTask.state!=='running')startTask(currentTask.id);
  const execution=await executeTask({...currentTask,state:'running'},lifecycleContext);if(execution.state==='completed'){markVerifying(currentTask.id);const verification=verifyResult(currentTask,execution);if(verification?.passed){completeTask(currentTask.id,verification.result??execution.result);return{status:'completed',task:store.get('tasks',currentTask.id),execution,verification};}const retry=retryDecision(currentTask,verification,execution.attempt??1);if(retry?.action==='retry')return executeTaskLifecycle(store.get('tasks',currentTask.id)??currentTask,{...lifecycleContext,attempt:(execution.attempt??1)+1});failTask(currentTask.id,verification?.error??'Verification failed');return{status:'failed',task:store.get('tasks',currentTask.id),execution,verification};}
  failTask(currentTask.id,execution.error??'Execution failed');return{status:'failed',task:store.get('tasks',currentTask.id),execution};
}
...
export function schedulerTick(){recoverStaleExecutions();return store.list('tasks').filter(t=>t.state==='queued').map(t=>t.id);}
