...
registerExecutor('internal.code',async({task,env,agentId})=>{
  ...
  if(!parsed){
    ...
    parsed={summary:`Code artifact generated without paid/cloud services. ${acceptance.join(' | ')}`,files:[{path:'generated/README.md',content:`MAULI L1 generated workspace\nTask: ${task.title??''}\nObjective: ${task.description??''}\nAcceptance: ${acceptance.join(' | ')}`}],tests:['node --test'],notes:['Deterministic L1 fallback used because AI runtime is unavailable.']};
  }
  const files=Array.isArray(parsed?.files)?parsed.files.filter(file=>file&&typeof file.path==='string'&&typeof file.content==='string'):[];
  const artifact=registerArtifact({projectId:task.projectId,taskId:task.id,agentId,type:'code-workspace',content:{summary:String(parsed?.summary??'Code artifact'),files,tests:Array.isArray(parsed?.tests)?parsed.tests:[],notes:Array.isArray(parsed?.notes)?parsed.notes:[]},metadata:{generatedBy:env?.AI?.run?'internal.code':'internal.code-fallback'}});
...
},{description:'Generates code workspace artifacts using the MAULI Intelligence Bus with a deterministic free fallback',risk:'low',scope:'internal',capabilities:['coding','software-development']});grantExecutor('internal.code','internal');
