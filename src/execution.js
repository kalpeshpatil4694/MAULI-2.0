...
function requiredToolNames(task) {
  return [...new Set((task?.toolNames ?? task?.requiredTools ?? []).map(String).filter(Boolean))];
}

async function authorizeRequiredTools(task, context, callTool) {
  const required = requiredToolNames(task);
  const results = [];
  for (const name of required) {
    ...
    const toolContext = {
      ...context,
      type: 'authorization-check', taskId: task.id, projectId: task.projectId,
      agentId: context.agentId, approved: Boolean(context.approved),
      approvalId: context.approvalId ?? null, allowExternal: Boolean(context.allowExternal)
    };
    const authorization = await callTool(name, toolContext);
    results.push({ name, authorization });
  }
  return results;
}
...
function grantedApprovalForTask(task) {
  return store.list('approvals').find(a => a.state === 'approved' && (a.taskId === task?.id || (!a.taskId && a.projectId === task?.projectId))) ?? null;
}
function recordExecutionMemory(task, execution) {
  const base = { scope: 'task', scopeId: task?.id ?? null, source: 'execution', tags: [task?.executor, ...(task?.requiredCapabilities ?? [])].filter(Boolean) };
  remember({ type: 'task_result', content: { taskId: task?.id ?? null, status: execution?.state ?? 'unknown', result: execution?.result ?? null, executionId: execution?.executionId ?? execution?.id ?? null }, importance: execution?.state === 'completed' ? 'normal' : 'high', ...base });
  if (execution?.state === 'completed') remember({ type: 'solution', content: { taskId: task?.id ?? null, summary: 'Task execution completed successfully.', executionId: execution?.executionId ?? execution?.id ?? null }, importance: 'normal', ...base });
  else if (execution?.error) remember({ type: 'error', content: { taskId: task?.id ?? null, error: execution.error }, importance: 'high', ...base });
}
function latestExecutionForTask(taskId) {
  return store.list('executions').filter(e => e.taskId === taskId).sort((a,b) => String(b.completedAt ?? b.startedAt ?? '').localeCompare(String(a.completedAt ?? a.startedAt ?? '')))[0] ?? null;
}
function completedLifecycleResponse(task) {
  const execution = latestExecutionForTask(task.id);
  const verification = execution ? verifyResult(task, execution) : null;
  return { status: 'completed', task, execution: execution ? publicExecution(execution) : null, verification: verification ?? { passed: true, result: task.result ?? task.output ?? null } };
}
...
export async function executeTask(task, context = {}) {
  ...
  if (existing && !context.forceRestart) {
    if (isStaleRun(existing)) {
      ...
      heartbeatExecution(existing.id);
  ...
  try {
    ...
    const completed = { ...store.get('runs', run.id) ?? run, state: 'completed', result, requiredTools, completedAt: now(), heartbeatAt: now(), recoverable: false, id: run.id };
    store.put('runs', completed); persistExecution(completed); recordExecutionMemory(task, completed); store.addEvent('execution.completed', completed);
    return publicExecution(completed);
  } catch (error) {
    const failed = { ...store.get('runs', run.id) ?? run, state: 'failed', error: error?.message ?? String(error), completedAt: now(), heartbeatAt: now(), recoverable: false, id: run.id };
    store.put('runs', failed); persistExecution(failed); recordExecutionMemory(task, failed); store.addEvent('execution.failed', failed);
    return publicExecution(failed);
  }
}
...
function recoverRun(run) {
  ...
  if (task && !['completed', 'cancelled'].includes(task.state)) {
    ...
    const nextState = attempts < Number(task.maxAttempts ?? MAX_RECOVERY_ATTEMPTS) ? 'queued' : 'failed';
    store.put('tasks', { ...task, state: nextState, blockedReason: nextState === 'queued' ? 'Recovered from stale execution' : task.blockedReason, error: nextState === 'failed' ? 'Execution lease expired after maximum recovery attempts' : task.error, updatedAt: timestamp, id: task.id });
