...
export function json(request) { const length = Number(request?.headers?.get?.('content-length') ?? 0); if (Number.isFinite(length) && length > 1048576) return Promise.resolve({}); return request.json().catch(() => ({})); }
export function route(path, method, expectedPath, expectedMethod = 'GET') { return method === expectedMethod && path === expectedPath; }
