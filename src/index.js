...
import { createCommand, getCommand, listRecentCommands, startCommand, resumePendingCommands } from './command-runtime.js';
...
const handler={async fetch(request,env,ctx){try{
  ...
  if(request.method==='GET'&&url.pathname==='/api/state'){
    const auth=requireFounder(request,env);if(!auth.ok)return fail(auth.error,auth.status);const [agents,projects,tasks,approvals,events]=hasD1(env)?await Promise.all([d1List(env,'agents'),d1List(env,'projects'),d1List(env,'tasks'),d1List(env,'approvals'),d1Events(env)]):[listAgents(),listProjects(),listTasks(),listApprovals(),store.recentEvents()];
    const safeProjects=projects.map(p=>{const pt=tasks.filter(t=>t.projectId===p.id);return p.state==='completed'&&!completionSafe(p,pt)?{...p,state:'active',completionGuard:'blocked-until-all-tasks-finish'}:p});return ok({agents,projects:safeProjects,tasks,approvals,tools:listTools(),artifacts:store.list('artifacts'),events,recoveredRuns});
  }
  if(request.method==='GET'&&url.pathname==='/api/command-center'){const auth=requireFounder(request,env);if(!auth.ok)return fail(auth.error,auth.status);return ok({models:modelRegistry.list({enabledOnly:true}),providers:providerSnapshot().providers,learning:learningSnapshot(),memoryCount:store.list('memory').length,production:productionSnapshot({env,recoveredRuns,store})})}
  if(request.method==='GET'&&url.pathname==='/api/commands'){const auth=requireFounder(request,env);if(!auth.ok)return fail(auth.error,auth.status);return ok({commands:await listRecentCommands(env,Number(url.searchParams.get('limit')||20))})}
  if(request.method==='GET'&&url.pathname.startsWith('/api/commands/')){const auth=requireFounder(request,env);if(!auth.ok)return fail(auth.error,auth.status);const command=await getCommand(env,url.pathname.split('/').pop());return command?ok({command}):fail('Command not found',404)}
  if(request.method==='GET'&&url.pathname==='/api/observer'){const auth=requireFounder(request,env);if(!auth.ok)return fail(auth.error,auth.status);const params={limit:url.searchParams.get('limit')??100,domain:url.searchParams.get('domain')??undefined,taskId:url.searchParams.get('taskId')??undefined,projectId:url.searchParams.get('projectId')??undefined,agentId:url.searchParams.get('agentId')??undefined,executionId:url.searchParams.get('executionId')??undefined,verificationId:url.searchParams.get('verificationId')??undefined,since:url.searchParams.get('since')??undefined};return ok({events:listObserverEvents(params),summary:observerSummary(params)})}
  if(request.method==='GET'&&url.pathname==='/api/observer/timeline'){const auth=requireFounder(request,env);if(!auth.ok)return fail(auth.error,auth.status);const limit=url.searchParams.get('limit')??200;const taskId=url.searchParams.get('taskId'),projectId=url.searchParams.get('projectId'),agentId=url.searchParams.get('agentId'),executionId=url.searchParams.get('executionId'),verificationId=url.searchParams.get('verificationId');if(taskId)return ok({timeline:getTaskTimeline(taskId,{limit})});if(projectId)return ok({timeline:getProjectTimeline(projectId,{limit})});if(agentId)return ok({timeline:getAgentTimeline(agentId,{limit})});if(executionId)return ok({timeline:getExecutionTimeline(executionId,{limit})});if(verificationId)return ok({timeline:getVerificationTimeline(verificationId,{limit})});return ok({timeline:listObserverEvents({limit})})}
  ...
  if(request.method==='POST'&&url.pathname==='/api/command'){const limit=checkRateLimit(request);if(!limit.ok)return fail(limit.error,limit.status,{retryAfter:limit.retryAfter});const body=await json(request);if(!body.command)return fail('Founder command is required',400);const auth=requireFounder(request,env);if(!auth.ok)return fail(auth.error,auth.status);const command=await createCommand(env,String(body.command).trim());startCommand(env,command,ctx?.waitUntil?.bind(ctx));return ok({commandId:command.id,state:command.state,command:command.command},202)}
...
export async function scheduled(event,env,ctx){await ensureSchema(env);store.configure(env);if(!store.hydrated)await store.hydrate();ensureBuiltinTools();seedAgents();const job=resumePendingCommands(env);if(ctx?.waitUntil)ctx.waitUntil(job);else await job}
