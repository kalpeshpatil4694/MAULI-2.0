...
import { ensureSchema, hasD1, d1List, claimBuildVersion, getBuildVersion, getUsageReport, cleanupD1 } from './db.js';
...
export default { async fetch(request, env) { try {
  ...
  if(request.method==='GET'&&url.pathname==='/api/state'){const limit=checkRateLimit(request);if(!limit.ok)return fail(limit.error,limit.status,{retryAfter:limit.retryAfter});const [agents,projects,tasks,approvals,events]=[listAgents(),listProjects(),listTasks(),listApprovals(),store.recentEvents()];return ok({agents,projects,tasks,approvals,tools:listTools(),artifacts:store.list('artifacts'),events,recoveredRuns});}
