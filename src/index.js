...
const dashboard = () => '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MAULI 2.0 Command Center</title><style>body{margin:0;font-family:system-ui;background:#080d18;color:#eef2ff}main{padding:24px;max-width:1200px;margin:auto}.card{background:#111a30;border:1px solid #29355c;border-radius:14px;padding:18px;margin-bottom:16px}button{padding:10px 14px;border:0;border-radius:8px;cursor:pointer}textarea{width:100%;min-height:110px;box-sizing:border-box;background:#080d18;color:#fff;border:1px solid #33406a;padding:10px;border-radius:8px}.result{white-space:pre-wrap;word-break:break-word}</style></head><body><main><div class="card"><h1>MAULI 2.0</h1><p>Autonomous AI Company — L1 Command Center</p><textarea id="cmd" placeholder="Founder command"></textarea><br><br><button onclick="send()">Send to MAULI</button> <button onclick="test()">Run L1 Self-Test</button><pre id="out" class="result"></pre></div><div class="card"><h2>Live State</h2><pre id="state" class="result"></pre></div></main><script>async function key(){return prompt("Founder API key")||""}async function send(){const command=document.getElementById("cmd").value.trim();if(!command)return;const k=await key();if(!k)return;const r=await fetch("/api/command",{method:"POST",headers:{"content-type":"application/json",authorization:"Bearer "+k},body:JSON.stringify({command})});document.getElementById("out").textContent=JSON.stringify(await r.json(),null,2);load()}async function test(){const k=await key();if(!k)return;const r=await fetch("/api/self-test",{headers:{authorization:"Bearer "+k}});document.getElementById("out").textContent=JSON.stringify(await r.json(),null,2)}async function load(){const r=await fetch("/api/state");document.getElementById("state").textContent=JSON.stringify(await r.json(),null,2)}load();setInterval(load,5000)</script></body></html>';
